...
def evaluate_signal(df: pd.DataFrame, config: ScalpConfig,
    ...
    # Select best signal — ALL conditions must be met
    ...
    if long_confidence >= short_confidence and long_conditions_met >= config.min_conditions:
        atr_val = atr[-1]
        result.signal = "LONG"
    ...
    elif short_confidence > long_confidence and short_conditions_met >= config.min_conditions:
        atr_val = atr[-1]
        result.signal = "SHORT"
        result.entry_price = current_price
        result.sl_price = current_price + (atr_val * config.sl_atr_mult)
    ...
    if result.signal != "NONE":
        ...
        if result.signal != "NONE" and result.confidence < config.min_confidence:
            result.signal = "NONE"
            result.confidence = 0
            result.conditions = {}
            result.condition_scores = {}
