...
PATCHES = {
    "core/testsynth/__init__.py": '''"""Test synthesis helpers for regression guards."""\n''',
...
def run_python_replay(file_path: str | Path) -> ReplayTestResult:
...
''',
    ...
    "core/testsynth/regression_guard.py": '''from __future__ import annotations
...
def _synthesized_regression_tests(candidate, file_path: str | None, semantic_before: dict[str, Any] | None):
    ...
    try:
        apply_text_replacement(file_path, code)
        replay = run_python_replay(file_path).to_dict()
        guard1 = check_failure_signature_removed(before_stderr, replay.get("stderr", "")).to_dict()
        guard2 = check_expected_exception_absent(before_exception_type, replay.get("stderr", "")).to_dict()
        combined = combine_regression_guards(
            check_failure_signature_removed(before_stderr, replay.get("stderr", "")),
            check_expected_exception_absent(before_exception_type, replay.get("stderr", "")),
        ).to_dict()
        ...
        return {
            ...
            "replay": replay,
            "guards": {
                "failure_signature_removed": guard1,
                "exception_absent": guard2,
                "combined": combined,
            },
        }
    finally:
...
''',
    ...
    "test_phase6_synth.py": '''#!/usr/bin/env python3
